...
func NewPost(db *pgxpool.Pool) *Post {
	s := Post{
		db: db,
	}
	return &s
...
func (s *Post) Create(ctx context.Context, post *model.Post) (*model.Post, error) {
	command := `
		INSERT INTO post
...
func (s *Post) Read(ctx context.Context, postID int) (*model.Post, error) {
	query := `
		SELECT
			post.*,
			blog.*
...
func (s *Post) ReadRecent(ctx context.Context, n int) ([]*model.Post, error) {
	query := `
		SELECT
			post.*,
			blog.*
...
func (s *Post) ReadRecentForUser(ctx context.Context, accountID int, n int) ([]*model.Post, error) {
	query := `
		SELECT
			post.*,
			blog.*
...
func (s *Post) Delete(ctx context.Context, postID int) error {
	command := `
		DELETE
		FROM post
		WHERE post_id = $1`
