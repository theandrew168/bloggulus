...
.PHONY: run
run:
	ENABLE_DEBUG_AUTH=1 go run github.com/bokwoon95/wgo@latest run -file .css -file .conf main.go

# run the app using the local-only config file
.PHONY: run-local
run-local:
	ENABLE_DEBUG_AUTH=1 go run github.com/bokwoon95/wgo@latest run -file .css -file .conf main.go -conf bloggulus.local.conf
