...
def runserver(s):
    ...
    # HACK: set EPOLLEXCLUSIVE if using epoll
    if isinstance(mux, selectors.EpollSelector):
        key = selectors.SelectorKey(s, s.fileno(), selectors.EVENT_READ, accept_client)
        mux._selector.register(s.fileno(), select.EPOLLIN | select.EPOLLEXCLUSIVE)
        mux._fd_to_key[s.fileno()] = key
    else:
